...
function ResortCard({ resort, prev, isFav, onToggleFav, departDate, returnDate, displayCurrency, preferredDeparture }: {
  ...
  const { next_3_days, following_7_days, total_10day_snow_cm } = resort.forecast;
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: t.fontSize.snowTotal, fontWeight: 800, color: t.colors.accentBlue, fontFamily: t.fonts.mono, lineHeight: 1 }}>
            {total_10day_snow_cm}<span style={{ fontSize: t.fontSize.subtext, color: t.colors.textMuted, fontWeight: 400 }}> cm</span>
          </div>
          <div style={{ fontSize: t.fontSize.subtext, color: t.colors.textMuted, marginTop: 2 }}>7-day total</div>
          <Delta current={total_10day_snow_cm} previous={prev?.forecast.total_10day_snow_cm} />
        </div>
      </div>
      ...
      <div>
        <div style={{ display: "flex", marginBottom: 6 }}>
          ...
          <div style={{ flex: 7, fontSize: t.fontSize.sectionLabel, color: t.colors.accentPurple, letterSpacing: 1, textTransform: "uppercase", paddingLeft: 8 }}>Following 7 days</div>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          ...
          <div style={{ flex: 4 }}><SnowBar days={following_7_days} color={t.bars.colorNext4} /></div>
        </div>
      </div>
      ...
      {resortMeta && (() => {
        ...
        const current10day = parseFloat(resort.forecast.total_10day_snow_cm);
        const pctOfAvg = avg > 0 ? Math.round((current10day / avg) * 100) : 0;
        ...
        {favResorts.map(r => (
          <div key={r.resort_id} onClick={() => router.push(`/resort/${r.resort_id}`)} style={{
            ...
            <div style={{ fontSize: t.fontSize.snowTotal - 4, fontWeight: 800, color: t.colors.accentBlue, fontFamily: t.fonts.mono }}>
              {parseFloat(r.forecast.total_10day_snow_cm).toFixed(1)}<span style={{ fontSize: 10, color: t.colors.textMuted }}> cm</span>
            </div>
...
function Top6Widget({ latest, favourites, sort, setSort, favsOnly, setFavsOnly }: {
  ...
  resorts = resorts.sort((a, b) => {
    if (sort === "snow") return parseFloat(b.forecast.total_10day_snow_cm) - parseFloat(a.forecast.total_10day_snow_cm);
...
function TableView({ latest, prev, favourites, onToggleFav, displayCurrency, arrivalAirportFilter }: {
  ...
  const sorted = [...latest.resorts].filter(r => {
    ...
    let av = 0, bv = 0;
    if (sort === "snow") { av = parseFloat(a.forecast.total_10day_snow_cm); bv = parseFloat(b.forecast.total_10day_snow_cm); }
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: t.fonts.body }}>
        ...
        <tbody>
          {sorted.map((r, i) => {
            ...
            const snowDiff = prevResort ? parseFloat(r.forecast.total_10day_snow_cm) - parseFloat(prevResort.forecast.total_10day_snow_cm) : null;
            return (
              <tr key={r.resort_id} onClick={() => router.push(`/resort/${r.resort_id}`)} style={{ background: i % 2 === 0 ? t.colors.tableBg : t.colors.historyRowBg, borderTop: `1px solid ${t.colors.borderSubtle}`, cursor: "pointer" }} onMouseEnter={e => (e.currentTarget.style.background = t.colors.statBg)} onMouseLeave={e => (e.currentTarget.style.background = i % 2 === 0 ? t.colors.tableBg : t.colors.historyRowBg)}>
                ...
                <td style={{ padding: `${t.table.cellPaddingV}px ${t.table.cellPaddingH}px`, whiteSpace: "nowrap" }}>
                  <span style={{ fontSize: t.fontSize.tableCell, fontWeight: 700, color: t.colors.accentBlue, fontFamily: t.fonts.mono }}>{r.forecast.total_10day_snow_cm} cm</span>
          ...
          <div key={run.fetched_at} style={{
            ...
            <div style={{ display: "flex", gap: 20, flex: 1, flexWrap: "wrap" }}>
              {run.resorts.map(r => {
                ...
                const diff = prevResort ? parseFloat(r.forecast.total_10day_snow_cm) - parseFloat(prevResort.forecast.total_10day_snow_cm) : null;
                return (
                  <div key={r.resort_id} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: t.fontSize.historyTime, fontWeight: 700, color: t.colors.textSecondary, fontFamily: t.fonts.mono }}>
                      {r.forecast.total_10day_snow_cm}<span style={{ fontSize: 9, color: t.colors.textMuted }}>cm</span>
        ...
        {latest ? (
          <>
              ...
              <>
                {(() => {
                  const filtered = [...latest.resorts]
                    ...
                    .sort((a, b) => {
                      if (top6Sort === "snow") return parseFloat(b.forecast.total_10day_snow_cm) - parseFloat(a.forecast.total_10day_snow_cm);
            ...
            <div style={{ display: "flex", gap: 20, fontSize: t.fontSize.subtext, color: t.colors.textMuted, paddingTop: 10, borderTop: `1px solid ${t.colors.borderSubtle}`, flexWrap: "wrap" }}>
              ...
              <span style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ display: "inline-block", width: 10, height: 10, borderRadius: 2, background: t.colors.accentPurple }} />Following 7 days</span>
